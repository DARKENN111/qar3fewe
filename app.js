...
const vk = new VKApi({
    token: "3a78495bfd65f95d8925c0573342d177899004f624cbc8bb8da0003824ed51f9d026882882435d0a45e7a"
});
...
function runBot(){
    ...
    request({
        ...
        if(typeof preUrl.response.game.video_id !== 'undefined'){
            vkBot.sendMessage('Клевер вот-вот начнется!')
            request({
            ...
            if(r.events.length !== 0){
                ...
                if(event.type == 'sq_question'){
                    ...
                    if(event.type == 'sq_game_winners'){
                        vkBot.sendMessage('Игра окончена.\n'+event.winners_num+' победителей разделили по '+event.winners+' руб.')
                    }else{
...
function winners(){
    var x = event.response.game.prize;
    var y = event.winners_num;
    var razd = x / y;
    Math.floor(razd);
};
...
function completedOCR(googleSearchResultText, pq) {
    ...
    if(/ НЕ /g.test(pq.q)){
        ...
        if(results[0].score+results[1].score+results[2].score !== 0){
            ...
            console.log('\n\nBEST ANSWER: '+results[0].text.join(' ')+' NUM: '+results[0].id);
            vkBot.sendMessage('Лучший ответ: '+results[0].text.join(' ')+' (Score: '+results[0].score+')\n\nНомер: '+results[0].id+'\n1.'+results[0].text.join(' ')+' (Score: '+results[0].score+')\n2.'+results[1].text.join(' ')+' (Score: '+results[1].score+')\n3.'+results[2].text.join(' ')+' (Score: '+results[2].score+')');
        }
    }else{
        if(results[0].score+results[1].score+results[2].score !== 0){
